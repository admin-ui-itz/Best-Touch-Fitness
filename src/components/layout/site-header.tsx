...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-charcoal-900/10 bg-cream-100/90 backdrop-blur supports-[backdrop-filter]:bg-cream-100/75">
        ...
        <nav aria-label="Primary" className="hidden items-center gap-1 md:flex">
          {navLinks.map((link) => (
            <Link
              ...
              className={`rounded-full px-3.5 py-2 text-sm font-semibold transition-colors hover:bg-charcoal-900/5 ${
                isActive(link.href) ? "text-charcoal-900 underline decoration-lime-500 decoration-[3px] underline-offset-8" : "text-ink-muted"
              }`}
            >
              {link.label}
            </Link>
      ...
      <div
        id={panelId}
        hidden={!open}
        className="border-t border-charcoal-900/10 bg-cream-100 md:hidden"
      >
